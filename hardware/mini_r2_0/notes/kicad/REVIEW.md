...
- `via_dangling`: 0件
- JLCPCB 向けルール適用後のDRC: 21 errors / 117 warnings

GND/GND2ゾーン復元により大量の未接続とGNDビアのdanglingは解消しました。残る `shorting_items` はモータドライバ U2/U3 周辺のフットプリント内ポリゴン/パッド接続として、KiCad GUIで実位置を確認します。
...
DRC の `shorting_items` は、同じ出力ピン接続そのものではなく、変換されたMP6551フットプリント内の無ネットポリゴンとパッド/配線の関係で出ている可能性が高いため、フットプリント側の銅形状に正しいネットを割り当てるか、不要な銅形状を削除する方針で確認します。
...
これらはJLCPCB向け Board Setup 適用後も残っています。`clearance` は銅箔間隔0.127 mm未満だけを実エラーとして扱います。`solder_mask_bridge` は細ピッチICやコネクタでソルダーマスクの島が残らない可能性を示すため、warningとして扱います。
...
| 基板 | 優先確認 |
| --- | --- |
| 本体基板 | `shorting_items`, モータドライバU2/U3フットプリント, JLCPCBクリアランス, ソルダーマスクブリッジ, IMU/FRAM SPI2, SWD/UART |
...
1. 本体基板の `shorting_items` 4件をKiCad GUIで優先確認します。
2. MP6551フットプリント内の無ネットポリゴンを、正しいネットへ割り当てるか削除するか判断します。
3. JLCPCB向けDRCで残る `clearance` / `solder_mask_bridge` を製造上許容できるか確認します。

...
def cleanup_ground_stitches(board: object) -> None:
    ...
    # These three central decouplers already share a contiguous F.Cu logic-GND
    # island with adjacent off-pad stitches.  Removing the redundant in-pad
    # stitches avoids forcing replacement holes into the dense SPI/+3V3 fanout.
    remove_via(board, "GND", (149.98, 121.00), 0.6, 0.3)
    # Preserve the known-good F.Cu/B.Cu island bridge at (152.60, 111.00).
    # Moving its 0402 decoupler upward by 0.40 mm clears the drill while the
    # annulus remains adjacent to the GND pad.
    c_mcu2 = board.FindFootprintByReference("C_MCU2")
    if c_mcu2 is None or not close(c_mcu2.GetPosition(), (152.70, 110.00)):
        raise RuntimeError("unexpected C_MCU2 placement")
    c_mcu2.SetPosition(pt(152.70, 109.60))
    delete_track(board, "+3V3", F, (152.70, 109.30), (152.70, 109.35), 0.20)
    delete_track(board, "+3V3", F, (153.80, 109.35), (152.70, 109.35), 0.20)
    add_track(board, "+3V3", F, (152.70, 108.95), (152.70, 109.30), 0.20)
    add_track(board, "+3V3", F, (152.70, 108.95), (153.80, 109.35), 0.20)
    add_track(board, "GND", F, (152.70, 110.25), (152.60, 111.00), 0.20)
    c_mcu_m0 = board.FindFootprintByReference("C_MCU-M0")
    if c_mcu_m0 is None or not close(c_mcu_m0.GetPosition(), (153.80, 110.00)):
        raise RuntimeError("unexpected C_MCU-M0 placement")
    c_mcu_m0.SetPosition(pt(153.80, 109.50))
    move_endpoint(board, "+3V3", F, (153.80, 109.35), (153.80, 108.85), "end")
    move_endpoint(board, "+3V3", F, (153.80, 109.35), (153.80, 108.85), "start")
    move_endpoint(board, "GND", F, (153.80, 110.65), (153.80, 110.15), "start")
    # Bridge the two filled logic-GND islands through the intentionally
    # pad-49-free MCU interior.  The path was obstacle-rastered at 0.16 mm
    # clearance; the layer transition is a normal 0.60/0.30 mm off-pad via.
    add_track(board, "GND", F, (150.90, 114.30), (148.90, 113.10), 0.20)
    add_track(board, "GND", F, (148.90, 113.10), (148.70, 112.90), 0.20)
    add_track(board, "GND", F, (148.70, 112.90), (148.65, 112.65), 0.20)
    add_track(board, "GND", F, (148.65, 112.65), (148.50, 111.10), 0.20)
    add_via(board, "GND", (148.50, 111.10), 0.60, 0.30)
    add_track(board, "GND", B, (148.50, 111.10), (148.30, 109.70), 0.20)
...
def main() -> int:
    ...
    cleanup_imu_supply(board)
    cleanup_front_sensor_toes(board)
    cleanup_c8(board)
    cleanup_fan_drain(board)
    cleanup_power_switch_return(board)
    refill(board)

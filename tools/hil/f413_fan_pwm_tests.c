...
HAL_StatusTypeDef HAL_TIM_PWM_Start(TIM_HandleTypeDef* h,unsigned c)
{ (void)c; assert(h == &htim10); active=true; return fail ? HAL_ERROR : HAL_OK; }
HAL_StatusTypeDef HAL_TIM_PWM_Stop(TIM_HandleTypeDef* h,unsigned c)
{ (void)c; assert(h == &htim10); active=false; return HAL_OK; }
int main(void)
{
 htim10.arr=999;
 ...
 puts("fan PWM: 50 percent, capability/stop/range refusal and failed-start cleanup PASS");
}

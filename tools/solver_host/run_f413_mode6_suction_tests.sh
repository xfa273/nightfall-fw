...
for TEST_MODE in 6 7; do
"${CC:-cc}" -DTEST_MODE="$TEST_MODE" -std=c11 -Wall -Wextra -Werror -Wno-unused-function -O1 -g \
  ...
  -lm -o "$TEST_OUT_DIR/f413_mode${TEST_MODE}_suction_tests"
ASAN_OPTIONS=detect_leaks=0:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 \
  "$TEST_OUT_DIR/f413_mode${TEST_MODE}_suction_tests"
done

...
static double speeds[4] = {500.0, 300.0, 100.0, 0.0};
...
static void linear_tables(NfSlalomContext *context) {
    speeds[2] = context->start_boundary_velocity_mm_s;
    require(speeds[2] > 0.0 && speeds[2] < speeds[1],
            "START speed requires different ordering from crawl/low/nominal");
    fprintf(stderr, "AUDIT ONLY: latest generic speeds %.9f/%.9f/%.9f mm/s; not execution model\n",

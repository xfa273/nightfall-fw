...
static bool f413_test_kerilab_2014_parity(const char* maze_path)
{
  ...
  CHECK(common.goal_entry_us == 10461309ULL);
  CHECK(common.stop_us == 10720867ULL);
  CHECK(compact.goal.goal_entry_us == 10761309UL);
  CHECK(compact.goal.stop_us == 11020867UL);
...
static bool f413_test_public_cleanup_and_metadata(const char* maze_path)
{
  ...
  CHECK(strstr(g_trace_output,
               "goal-entry=10.761309 s stop=11.020867 s") != NULL);
  CHECK(strstr(g_trace_output,

...
static bool f413_test_kerilab_2014_parity(const char* maze_path)
{
  ...
  CHECK(common.goal_entry_us == 10262694ULL);
  CHECK(common.stop_us == 10522252ULL);
  CHECK(compact.goal.goal_entry_us == 10562694UL);
  CHECK(compact.goal.stop_us == 10822252UL);
...
static bool f413_test_public_cleanup_and_metadata(const char* maze_path)
{
  ...
  CHECK(strstr(g_trace_output,
               "goal-entry=10.562694 s stop=10.822252 s") != NULL);
  CHECK(strstr(g_trace_output,

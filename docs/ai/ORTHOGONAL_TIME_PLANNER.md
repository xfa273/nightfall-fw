...
既存変換へ文法validatorを追加した。内蔵迷路のmode 2 / case 8では、現時点で
`[201, 901, 0]` を生成し、斜め進入状態のまま終端することを再現できる。

```sh
tools/solver_host/run_solver_host.sh --mode 2 --case 8 --assert-valid
```

これは今回の直交プランナには流入しない。斜めプリミティブを追加する段階で、
既存変換の修正と回帰fixtureとして使う。

## 次段階へ残す事項

- 調整済み45/135/V90と斜め直線を同じ状態グラフへ追加する
- ターンごとの掃引領域／クリアランスマスクを追加する
- `geometry=` に表示する公称ターン変位と論理アンカーの残差をターン調整後に再評価し、
  実機反映を許可する閾値を決める
- 連続raised-cosineモデルとF413の1 ms離散プロファイルの時刻差を定量化する
- 公称時間と動画・ログから得た実測時間の誤差を比較する
- 小迷路の全動作列挙とDijkstra結果を照合する

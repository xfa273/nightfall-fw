...
- mini_r3 mode7吸引試験（2026-09-22 JST、t0.21）: 小回り1.4/大90・180 2.0/斜め1.5 m/s、fan100%、既存FAN_ONゲイン・1200ms ramp/300ms待機を継承。PR21理想simでalpha/入口出口を作成、mode omega3000 deg/s、左右16ターン終点誤差0.925mm以下。case0をmode6と同順のsub0小回り/sub1大90/sub2大180へ。11:31小回りログは前後直進1.5/コア1.2指令、旋回瞬間duty93%を確認。F413共通のターン試験だけ経路内ターン設定から直進上限を導出しmode6/sub0全profile1.2、mode7/sub0全profile1.4へ。通常最短・直進試験の上限は維持。mode7直進2m/s・45m/s²/斜め1.5m/s・25m/s²、各2736最短接続、全10 subのphase速度、production session開始終了、機体/既存path219/route14210・両MCU build PASS。実機操作なし、斜め保守的壁余裕未達・高速追従未確認。詳細 `docs/MINI_R3_MODE7_SUCTION_TUNING.md`。

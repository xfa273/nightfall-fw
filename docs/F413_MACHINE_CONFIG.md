...
| mini_r3_0_unit002 | mini / `0x00030000` / 2 | `0x00030001` / `mini-r3-wall-centre-t0.5`（暫定機種profile） | Low（標準、モータ未実装・未検証） | 設計上あり |

両機とも現時点は右前進IN2 High、encoder符号 L=+1/R=-1、TIM2 PSC=0/ARR=1000。
r3の左配線反転は**機種標準ではなくunit001の上書き**。
2026-09-20追加のunit002には、この反転を引き継がない。
unit002のMCU UIDは `001D0038-32345108-36383936`。新基板でIMUは実装済み、
壁センサ・エンコーダ・モータ・ブザーは未実装として非駆動bring-upする。
機種共通profileのLUT/走行値は暫定値であり、unit002の距離精度・走行性能は未検証。
unit001のFRAM校正値はコピーせず、センサ実装後に個体校正する。

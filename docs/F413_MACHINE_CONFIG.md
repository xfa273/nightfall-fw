...
| mini_r3_0_unit001 | mini / `0x00030000` / 1 | `0x00030001` / `mini-r3-2s-fanoff-t0.10`（共通値、故障機の駆動は禁止） | High（左配線を反転済み） | あり |
| mini_r3_0_unit002 | mini / `0x00030000` / 2 | `0x00030001` / `mini-r3-2s-fanoff-t0.10`（ゲイン/LUT維持、停止進入修正・実機反映待ち） | High（unit001同様の左配線を確認） | あり |

登録済み各機とも現時点は右前進IN2 High、encoder符号 L=+1/R=-1、TIM2 PSC=0/ARR=1000。
r3の左配線反転は**機種標準ではなく各個体の上書き**。
unit002は2026-09-20登録時は標準Lowとし、2026-09-21に実装後の目視とencoderで
左だけ逆回転を確認したため、ユーザ指示でunit001同様のHighへ変更した。
encoder符号は変更せず、以後は正常な方向確認手段として扱う。
unit002のMCU UIDは `001D0038-32345108-36383936`。IMU・壁センサ・エンコーダ・
走行モータ・吸引モータは実装済みで、吸引動作もユーザ確認済み。
unit002の非吸引ゲイン・基本走行は2026-09-21ユーザ確認済み。前壁補正・
距離精度の独立検証、全mode/速度域・吸引・3Sの資格確認を意味しない。
2026-09-21、同じ壁センサをunit001から移植したユーザの指示により、遮光後の
sensor校正68B（offset FR82/FL66/R27/L43）をunit002へ暫定コピー済み。
校正コピー時の距離LUTは共通t0.5で、後述のt0.9で前壁のみ拡張。
unit001の無効な診断用distance blobはコピーしない。
横壁中央基準は保存0のためL1941/R1989の暫定値で、実機確認・調整が必要。
適用範囲と検証は `docs/MINI_R3_UNIT002_BRINGUP.md` を参照。
2026-09-21の浮上粗調整t0.6からt0.7で直進のIMU併用へ戻し、ユーザの床上調整後に
t0.8として速度P0.24・角速度P0.45を保存。他のゲインと角速度profile中のencoder
例外は維持。同日の追加測定を受領しt0.9で前壁40..110mmへ拡張、ゲインは不変。
このLUT更新の実機書込・前壁補正の床上追試、吸引・3Sは未検証。
unit001のU2故障による駆動禁止は解除しない。保存点は
`stable/mini/unit002/s20260921-mini-r3-2s-fan-off/`。
詳細 `docs/MINI_R3_UNIT002_TRANSLATION_TUNING.md`。
旧 `NIGHTFALL_F413_MOTOR_LEFT_FORWARD_IN2_HIGH` ビルド指定は廃止し、指定時はコンパイルエラー。

r2の既存数値は維持した。r3はr2の走行値を独立コピーしたprofileである。
2026-09-06にユーザーが同一外形・足回り構成を確認したため、寸法・encoder CPRを継承する。
unit002の非吸引負荷時ゲインはユーザ調整済み。有効タイヤ径の独立実測や
側壁制御基準値、未試験条件の確認は別途。ターン前壁補正の換算と追加測定条件は
`docs/MINI_R3_FRONT_TURN_REFERENCE.md`を参照。
前壁FR/FL/合計LUTは遮光後の09-12測定40..75mmと09-21追加測定80..110mmを
合成した15点を使用し、80mmは最新FR599/FL568へ置換（90mmは422/422）。
前壁合わせ目標45mm、近すぎ判定42.5mmを維持する。遮光前の85..110mmの点は混ぜない。profileの
`front_distance_body_centre` は前壁だけの基準を表し、起動・UART `:`・CSV metaにも表示する。
09-12の側壁測定L/R各12点（中心基準23..80mm）は `side_distance_lut.c` に維持し、
`side_distance_body_centre` はtrueを維持する。45mm時のR/Lは640/733。
側壁距離は診断表示用で、横壁制御・壁切れ判定は引き続きADC deltaを使用する。
生値閾値・ゲイン・FRAMの側壁基準値は変更しない。距離有効判定のdelta>300も保持するため、
測定範囲内でもL/Rとも65..80mmの点はlow-signalとなる。
新規の距離テーブルは機体中心基準で採取する。mini_r2は旧基準のLUT/目標7mmを保持し、
旧測定原点が不明なら中心基準で再測定してから移行する。
基準や元LUTの識別子を持たない既存v1 FRAM warpは中心基準profileでは読込適用しない

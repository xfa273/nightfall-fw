...
| mini_r3_0_unit001 | mini / `0x00030000` / 1 | `0x00030001` / `mini-r3-wall-centre-t0.4` | High（左配線を反転済み） | あり |
...
前壁FR/FL/合計LUTは2026-09-06の治具測定15点（機体中心から壁まで40..110mm）を使用し、
前壁合わせ目標を45mm、近すぎ判定を42.5mmへ変更した。profileの
`front_distance_body_centre` は前壁だけの基準を表し、起動・UART `:`・CSV metaにも表示する。
同日の側壁測定でL/R各12点（中心基準23..80mm）を別の `side_distance_lut.c` へ追加し、
`side_distance_body_centre` もtrueとした。前壁LUT/目標は維持する。
側壁距離は診断表示用で、横壁制御・壁切れ判定は引き続きADC deltaを使用する。
生値閾値・ゲイン・FRAMの側壁基準値は変更しない。距離有効判定のdelta>300も保持するため、
測定範囲内でもL70..80mm、R65..80mmの点はlow-signalとなる。

...
- Step39: ユーザ実測の青ラベル面10mm・赤ラベル面2mm（迷路床基準、水平基線24mm）をtracking artifactへ固定し、床／ArUco基準面homographyで生じる位置依存視差を各ラベル面へ別々に戻すhost校正系を追加した。固定カメラの中心XY・高さは走行軌跡から自己推定せず、盤面四隅相当の4静止姿勢でfitし中央1姿勢をheld-out検証する。fit span、凸包面積、静止時間/安定性、fit/held-out残差、物理的カメラ高さを全て通ったgeometryだけを許可し、既存CSVに残る青赤canonical座標から新しい補正CSVと入力hash付きsidecarを生成する。`turn_video_tune --propose-fit`と`turn_clearance video --registration-mode absolute`は全入力に検証済みsidecarが無ければfail closedとし、normalized未補正軌跡は診断専用に維持した。Pixel recorder 0.5.6は今後のPnP再現性のため、CaptureResultの焦点距離・intrinsic・distortion・crop・zoom・active physical camera IDとstatic sensor geometryをadditiveに保存する。既存27本は生ラベル座標を保持するため、同じカメラ位置・マーカ位置・crop・physical lensで撮影されたバッチなら校正後に再利用できる。ただし現行sidecarのsetup同一性はoperator確認であり自動fingerprintではない。現時点ではArUco上面の床からの高さが未入力なのでabsolute再解析・firmware parameter変更は行わない。

...
`mini_v1_1` と `classic_v2` を **共通コード + params差分のみ**で運用する前提を含みます。
...
### 4.4 `params/<variant>/params.h` の変更ルール（推奨）

- `params/<variant>/params.h` を編集した場合は、PR本文に以下を必ず書く
  - 変更項目（どのマクロ/値を変えたか）
  - 変更理由（狙い）
  - 実機確認結果（未実施なら未実施と明記）
- `mini_v1_1` と `classic_v2` の **両方に存在すべきパラメータ**（例: 壁切れ判定など）は、原則として両方に揃える
  - 片側だけ差を付ける場合は、理由（ハード差/センサ差/モータ差など）を明記する
- パラメータの意味/単位を変える場合は互換性が壊れるため、FW Version の扱い（MAJOR）も合わせて検討する
...
- `main`:
  ...
  - `main` に入れるPRは、原則として `mini_v1_1` と `classic_v2` の両方がビルドできること
- `feature/<topic>`:
...
ログ取得・可視化・書き込みなどの **ホスト側ツール（`tools/` 配下）** は、実験ブランチに混ざると取り込み時に漏れやすいです。
今後は以下のルールで運用します。

- `tools/` の変更は **必ず** `feature/tools/<topic>` または `fix/tools/<topic>` で行う
- 実験ブランチ（`exp/*`）にツール変更が必要になった場合は、
  - ツール修正は `feature/tools/*` / `fix/tools/*` にコミットし、先に `main` へマージする
  - 実験ブランチ側は `main` の取り込み（merge/rebase）で追従する
- 原則として、1つのPRに `platform/`（ファーム本体）と `tools/` の変更を混在させない
  - 例外が必要な場合は、PR本文に理由と影響範囲を明記する
  - 例外で混在する場合でも、コミットは可能な限り `tools/` と `platform/` で分ける

推奨:
- GitHub Actions等で「`platform/**` と `tools/**` が同一PRで変更されたら落とす」チェックを入れる

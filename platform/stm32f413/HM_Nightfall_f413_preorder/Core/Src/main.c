...
static volatile uint16_t g_wall_adc_dma_off[9];
static volatile uint16_t g_wall_adc_dma_on[9];
...
static void nightfall_wall_sensor_load_params(void);
static bool nightfall_wall_sensor_start_async(void);
static void nightfall_wall_sensor_tim6_tick(void);
static bool nightfall_wall_sensor_latest_snapshot(nightfall_wall_sensor_snapshot_t* out);
...
static bool g_search_step_session_active = false;
...
static uint16_t nightfall_wall_sensor_subtract_u16(uint16_t on, uint16_t off, uint16_t offset)
{
...
static HAL_StatusTypeDef nightfall_wall_sensor_adc_dma_start(volatile uint16_t* dst)
{
  ...
  return HAL_ADC_Start_DMA(&hadc1, (uint32_t*)dst, 9U);
}
...
static bool nightfall_wall_sensor_start_async(void)
{
  nightfall_wall_sensor_load_params();
...
static void nightfall_wall_sensor_tim6_tick(void)
{
  ...
  switch (g_wall_sensor_adc_phase & 0x07U)
  {
    case 0U:
      ...
      break;
    case 1U:
      st = nightfall_wall_sensor_adc_dma_start(g_wall_adc_dma_off);
      start_dma = 1U;
      break;
    case 2U:
      ...
      break;
    case 3U:
      st = nightfall_wall_sensor_adc_dma_start(g_wall_adc_dma_on);
      start_dma = 1U;
      break;
    case 4U:
                                                    ...
                                                    uint32_t timestamp_ms)
{
  ...
  if (nightfall_wall_sensor_latest_snapshot(&wall))
  {
...
static bool nightfall_wall_sensor_read_snapshot(nightfall_wall_sensor_snapshot_t* out)
{
  return nightfall_wall_sensor_latest_snapshot(out);
}
...
static void nightfall_search_write_map_cell(uint8_t x, uint8_t y, uint8_t dir, uint16_t relative_wall_info)
{
  ...
  m_temp = (relative_wall_info >> (dir & 0x03U)) & NIGHTFALL_F413_MAZE_WALL_KNOWN_MASK;
  if ((x == START_X) && (y == START_Y))
  {
    m_temp |= NIGHTFALL_F413_MAZE_START_FORCED_WALLS;
  }
...
static void nightfall_search_step_session_reset(void)
{
  g_search_step_session_active = false;
  mouse.x = START_X;
  mouse.y = START_Y;
  mouse.dir = 0U;
}

static void nightfall_search_step_advance_position(void)
{
...
static float nightfall_search_step_turn_target_deg(uint8_t rel)
{
  switch (rel)
  {
    case 1U:
      return -NIGHTFALL_F413_SEARCH_STEP_TURN_DEG;
    case 2U:
      return 2.0f * NIGHTFALL_F413_SEARCH_STEP_TURN_DEG;
    case 3U:
      return NIGHTFALL_F413_SEARCH_STEP_TURN_DEG;
    default:
      return 0.0f;
...
static void nightfall_run_search_step_once(void)
{
  ...
  if (!g_search_step_session_active)
  {
    nightfall_search_map_init_empty();
    mouse.x = START_X;
    mouse.y = START_Y;
    mouse.dir = 0U;
    g_search_step_session_active = true;
  ...
  is_turn = (next_rel != 0U);
  turn_target_deg = nightfall_search_step_turn_target_deg(next_rel);
  ...
  if (is_turn)
  {
    ...
    abort_reason = nightfall_wait_ctrl_target(turn_target_deg, true, &guard,
  ...
  if (abort_reason == NIGHTFALL_RUN_ABORT_NONE)
  {
    ...
    {
      nightfall_search_step_advance_position();
    }
...
static void nightfall_run_search_map_probe_once(void)
{
  ...
  if (!nvm_maze_load_map(loaded, NIGHTFALL_F413_SEARCH_MAP_CELL_COUNT))
  {
...
static void nightfall_op_enter_selection(void)
{
  ...
  {
    if ((g_op_case == 0U) &&
        (((g_op_mode >= 1U) && (g_op_mode <= 7U)) || (g_op_mode == 9U)))
    {
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
  ...
  switch (g_wall_sensor_adc_phase & 0x07U)
  {
    case 3U:
    {
      ...
      g_wall_adc_r = nightfall_wall_sensor_subtract_u16(r_on, r_off, g_wall_offset_r);
      g_wall_adc_l = nightfall_wall_sensor_subtract_u16(l_on, l_off, g_wall_offset_l);
      HAL_GPIO_WritePin(IR_R_GPIO_Port, IR_R_Pin, GPIO_PIN_RESET);
      HAL_GPIO_WritePin(IR_L_GPIO_Port, IR_L_Pin, GPIO_PIN_RESET);
    ...
    case 7U:
    {
      ...
      g_wall_adc_fr = nightfall_wall_sensor_subtract_u16(fr_on, fr_off, g_wall_offset_fr);
      g_wall_adc_fl = nightfall_wall_sensor_subtract_u16(fl_on, fl_off, g_wall_offset_fl);
